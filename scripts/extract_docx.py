"""Extract ordered blocks (paragraphs + runs formatting + images) from gce level 1.docx."""
import json
import sys
import zipfile
...
def attr_val(elem: ET.Element, local: str) -> str | None:
    """Attribute may be {W}val or val depending on parser."""
    if elem is None:
        return None
    full = f"{{{W}}}{local}"
...
def run_text_and_props(r: ET.Element) -> tuple[str, bool, str | None]:
    """Return (plain_text, bold, color_hex_or_none). color is 6-char RRGGBB upper."""
    parts: list[str] = []
    for child in r:
        lt = local_tag(child)
        if lt == "t":
...
def iter_runs_in_paragraph(p: ET.Element):
    """Document-order runs (handles w:hyperlink, w:ins, w:smartTag, w:sdt, etc.)."""
...
def paragraph_to_block(p: ET.Element, rid_to_target: dict[str, str]) -> dict:
    ...
    # Stripped text for search / copy helpers; runs keep original spacing for slice alignment
    ...
    # Fallback: no structured runs (shouldn't happen) — flatten
...
def main() -> int:
    ...
    doc_path = root_dir / "gce level 1.docx"
